...
async function executeFetch(context, fetch, results, _path, traceNode) {
    ...
    return opentelemetry_1.tracer.startActiveSpan(opentelemetry_1.OpenTelemetrySpanNames.FETCH, { attributes: { service: fetch.serviceName } }, async (span) => {
        try {
            ...
            if (!fetch.requires) {
                const dataReceivedFromService = await sendOperation(context, fetch.operation, variables, fetch.operationName);
                ...
                const dataReceivedFromService = await sendOperation(context, fetch.operation, { ...variables, representations }, fetch.operationName);
    ...
    async function sendOperation(context, source, variables, operationName) {
        var _a, _b;
        let http;
        if (traceNode) {
            http = {
